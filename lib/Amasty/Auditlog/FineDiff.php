...
include_once 'FineDiffDeleteOp.php';
...
class FineDiff {
    ...
    private static function renderToTextFromOpcode($opcode, $from, $from_offset, $from_len) {
        if ( $opcode === 'c' || $opcode === 'i' ) {
            Mage::helper('ambase/utils')->_echo(substr($from, $from_offset, $from_len));
        }
    }

    private static function renderDiffToHTMLFromOpcode($opcode, $from, $from_offset, $from_len) {
        if ( $opcode === 'c' ) {
            Mage::helper('ambase/utils')->_echo(htmlentities(substr($from, $from_offset, $from_len)));
        }
        else if ( $opcode === 'd' ) {
            ...
            Mage::helper('ambase/utils')->_echo('<del>' . htmlentities($deletion) . '</del>');
        }
        else /* if ( $opcode === 'i' ) */ {
            Mage::helper('ambase/utils')->_echo('<ins>' . htmlentities(substr($from, $from_offset, $from_len)) . '</ins>');
        }
    }
}
