...
class Hm_Testimonial_Block_Testimonial extends Mage_Core_Block_Template
{
    ...
    public function getRecents()     
    {
	    ...
	    //$collection->setPageSize(Mage::getStoreConfig('hm_testimonial/general/num_testimonial') ? Mage::getStoreConfig('hm_testimonial/general/num_testimonial'):5);
      $total = 1;//Mage::getStoreConfig('hm_testimonial/general/total'); 
      if (!$total) $total = 1; 
      
	    $collection->setPageSize($total)->setOrder('rand()');
    ...
    /*******
     * Added for contet carousel testiminals at homepage
     ****/
    ...
    public function getAllRecents()     
    {
    	if (!Mage::app()->isSingleStoreMode()) {
		    $collection = Mage::getModel('testimonial/testimonial')->getCollection()
		    ->addStoreFilter(Mage::app()->getStore()->getId())
		    ->addFieldToFilter('media',array('neq' => ''))   
            ->setOrder('sort_order', 'asc');
    	}else{
    		$collection = Mage::getModel('testimonial/testimonial')->getCollection()->addFieldToFilter('media',array('neq' => ''))->setOrder('sort_order','asc');
    	}
$collection = Mage::getModel('testimonial/testimonial')->getCollection()->addFieldToFilter('is_homepage',1);
	    $route = Mage::helper('testimonial')->getRoute();
	    Mage::getSingleton('testimonial/status')->addEnabledFilterToCollection($collection);
