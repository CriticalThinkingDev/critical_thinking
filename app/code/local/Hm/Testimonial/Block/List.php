...
class Hm_Testimonial_Block_List extends Mage_Core_Block_Template
{
    ...
    public function getTestimonials($isTeacher = '2')     
    {
    	if (!Mage::app()->isSingleStoreMode()) {
		    $collection = Mage::getModel('testimonial/testimonial')->getCollection()
		    ->addStoreFilter(Mage::app()->getStore()->getId())
		     ->addFieldToFilter('is_teacher',array('eq' => $isTeacher))    ->setOrder('sort_order', 'asc');
    	}else{
    		$collection = Mage::getModel('testimonial/testimonial')->getCollection()->addFieldToFilter('is_teacher',array('eq' => $isTeacher)) ->setOrder('sort_order','asc');
    	}
