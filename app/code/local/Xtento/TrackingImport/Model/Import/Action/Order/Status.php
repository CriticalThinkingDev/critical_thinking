 ...
 * Last Modified: 2015-11-30T17:03:35+01:00
...
class Xtento_TrackingImport_Model_Import_Action_Order_Status extends Xtento_TrackingImport_Model_Import_Action_Abstract
{
    public function update()
    {
        ...
        if (!$statusSet && !@empty($updateData['order_status_history_comment'])) {
            ...
            $this->setHasUpdatedObject(true);
        } else if ($this->getActionSettingByFieldBoolean('send_order_update_email', 'enabled')) {
        ...
        if ($this->getActionSettingByFieldBoolean('send_order_update_email', 'enabled')) {
            ...
            $this->addDebugMessage(Mage::helper('xtento_trackingimport')->__("Order '%s': Order update email dispatched.", $order->getIncrementId()));
            $this->setHasUpdatedObject(true);
        }
