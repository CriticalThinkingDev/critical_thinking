		...
		if (!is_null($value)) {
			...
			if ($this->getColumn()->hasData('type')) {
				if ($this->getColumn()->getData('type')=='form') {
					$html.= '<input type="text" class="input-text ' . $this->getColumn()->getValidateClass() . '" name="downloadable['.( $this->getColumn()->getName() ? $this->getColumn()->getName() : $this->getColumn()->getId() ).']['.$row->getItemId().']" value="" />';
				}
			}
		}
		return $html;
