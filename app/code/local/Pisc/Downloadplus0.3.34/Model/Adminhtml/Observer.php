...
class Pisc_Downloadplus_Model_Adminhtml_Observer
{
	...
	{
		return $observer->getRequest()->getParam('store');
	}
		...
		if ($links = $this->getValue($downloadable['productlink'], false)) {
			...
			foreach ($links as $link) {
				...
				if ($link['is_delete']==1) {
					...
					$download->setLinkTitle($link['title']);
					$download->setLinkType($link['type']);
					$download->setLinkUrl($link['link_url']);
					
					if ($download->getLinkType() == Pisc_Downloadplus_Helper_Download::LINK_TYPE_AWSS3 && isset($link['amazon_s3_bucket']) && isset($link['amazon_s3_file'])) {
					...
					$download->getDownloadDetail()->setDetail($link['description']);
		...
		if ($statuses = $this->getValue($downloadable['status'], false)) {
			foreach ($statuses as $key=>$status) {
				if ($link = Mage::getModel('downloadplus/link_purchased_item')->load($key)) {
					if ($link->getStatus()==Pisc_Downloadplus_Model_Link_Purchased_Item::LINK_STATUS_AVAILABLE && $status==Pisc_Downloadplus_Model_Link_Purchased_Item::LINK_STATUS_EXPIRED) {
						$link->setStatus($status);
						$link->save();
					}
					if (($link->getStatus()==Pisc_Downloadplus_Model_Link_Purchased_Item::LINK_STATUS_EXPIRED && $status==Pisc_Downloadplus_Model_Link_Purchased_Item::LINK_STATUS_AVAILABLE)
