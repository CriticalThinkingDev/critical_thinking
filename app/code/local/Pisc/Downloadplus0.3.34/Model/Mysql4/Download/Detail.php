...
class Pisc_Downloadplus_Model_Mysql4_Download_Detail extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    public function addFileToFilter($file)
    {
    	...
    	if ($helper->isInPath($file, Mage::getModel('downloadable/link')->getBasePath())) {
    		$this->addPathToFilter(Mage::getModel('downloadable/link')->getBasePath());
    		$file = $helper->removePath($file, Mage::getModel('downloadable/link')->getBasePath());
    	}
    	if ($helper->isInPath($file, Mage::getModel('downloadable/link')->getBaseSamplePath())) {
    		$this->addPathToFilter(Mage::getModel('downloadable/link')->getBaseSamplePath());
    		$file = $helper->removePath($file, Mage::getModel('downloadable/link')->getBaseSamplePath());
    	}
    	if ($helper->isInPath($file, Mage::getModel('downloadable/sample')->getBasePath())) {
    		$this->addPathToFilter(Mage::getModel('downloadable/sample')->getBasePath());
    		$file = $helper->removePath($file, Mage::getModel('downloadable/sample')->getBasePath());
    	}
