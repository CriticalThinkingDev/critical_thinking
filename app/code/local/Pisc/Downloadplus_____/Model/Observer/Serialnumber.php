...
class Pisc_Downloadplus_Model_Observer_Serialnumber
{
	...
	public function eventProductAssignSerialnumber($observer)
	{
		...
		$purchasedLinks = Mage::getResourceModel('downloadplus/link_purchased_item_collection')
							->addFieldToFilter('order_item_id', array('in'=>$orderItem->getId()));
		...
		foreach ($purchasedLinks as $purchasedLink) {
			...
			*/
			...
			if ($purchasedLink->getLinkId() && !$helper->hasSerialnumbersDeactivated($orderItem->getProductId()) && $helper->hasSerialnumbers($orderItem->getProductId(), $purchasedLink->getLinkId()) && 
				...
				$serials = Mage::getModel('downloadplus/link_purchased_item_serialnumber')
								->getCollection()
								->getByPurchasedItemId($purchasedLink->getId());
