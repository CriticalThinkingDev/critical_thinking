...
class Pisc_Downloadplus_Model_Customer_Download extends Mage_Core_Model_Abstract
{
    ...
    public function getLinkPurchasedItemCollection()
    {
    	if (is_null($this->_items)) {
	        $purchasedIds = $this->getLinkPurchasedCollection()->getAllIds();
	        ...
	        foreach ($purchasedItems as $_item) {
				...
				if ($data) {
	        		$_item->setData('newest_download_timestamp', date('Y-m-d H:i:s', strtotime($data)));
				} else {
					$_item->setData('newest_download_timestamp', null);
				}
	        }
