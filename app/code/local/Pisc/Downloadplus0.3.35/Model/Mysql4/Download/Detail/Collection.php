...
class Pisc_Downloadplus_Model_Mysql4_Download_Detail_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    public function addStoreToFilter($store)
    {
    	...
    	$this->addFieldToFilter('store_id', $this->_store_id);
    	return $this;
    }
    ...
    public function getActiveFiles()
    {
    	...
    	// Get all current associated downloadable files
    	$currentFiles = array_merge(
	    					Mage::getModel('downloadplus/downloads')
								->setProduct($this->_product)
								->getLinkUpdates(),
	    					Mage::getModel('downloadplus/downloads')
								->setProduct($this->_product)
								->getLinkSampleUpdates(),
							Mage::getModel('downloadplus/downloads')
								->setProduct($this->_product)
								->getSampleUpdates()
						);

		// Add Files and Details to Collection
		foreach ($currentFiles as $file) {
			$item = Mage::getModel('downloadplus/download_detail')->loadByFile($file['pathfile']);
			$item->makeActive();
			$collection[] = $item;
		}
    ...
    public function getHistoricalFiles()
    {
    	...
    	if (is_null($this->_store_id)) { $where[] = "store_id IS NULL"; } else { $where[]="store_id='".$this->_store_id."'"; }
    	$where[] = "link_customer_item_id IS NULL";
    	$where[] = "link_product_item_id IS NULL";

    	$sql = $this->getResource()->getReadAdapter()
					->select()
		...
		if ($historicalFiles = $this->getResource()->getReadAdapter()->fetchAll($sql)) {
			foreach ($historicalFiles as $file) {
