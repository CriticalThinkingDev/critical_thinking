...
class Pisc_Downloadplus_DownloadController extends Mage_Downloadable_DownloadController
{
	...
	protected function _processDownload($resource, $resourceType, $resourceObject=null)
    {
        ...
        if (Mage::getModel('downloadplus/config')->isDownloadableTrackProduct() && ($resourceObject instanceof Mage_Downloadable_Model_Link_Purchased_Item)) {
        	if ( $resourceObject->getId() ) {
        		$log = Mage::getModel('downloadplus/log_link');
        		$log->trackPurchasedLink($resourceObject);
        	}
        }
        if (Mage::getModel('downloadplus/config')->isDownloadableTrackProduct() && ($resourceObject instanceof Mage_Downloadable_Model_Link)) {
        	if ( $resourceObject->getId() ) {
        		$log = Mage::getModel('downloadplus/log_link');
        		$log->trackLinkSample($resourceObject);
        	}
        }
        if (Mage::getModel('downloadplus/config')->isDownloadableTrackSample() && ($resourceObject instanceof Mage_Downloadable_Model_Sample)) {
