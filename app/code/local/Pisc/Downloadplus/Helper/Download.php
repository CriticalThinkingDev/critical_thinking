...
class Pisc_Downloadplus_Helper_Download extends Mage_Downloadable_Helper_Download
{
	...
	const LINK_TYPE_EDITIONGUARD = 'editionguard';
	const LINK_TYPE_BUILDER = 'builder';
	const LINK_TYPE_MAGAZINE = 'magazine';
	
	/* Filter these Link Types on default 'My Downloadable Products' frontend */
	const FRONTEND_FILTER_LINK_TYPES = 'magazine';
	
	public function setResource($resourceFile, $linkType = self::LINK_TYPE_FILE)
	{
		parent::setResource($resourceFile, $linkType);

		/*
		 * Treat some link types as FILE
		 */
		if ($this->_linkType==self::LINK_TYPE_FILELOCAL || $this->_linkType==self::LINK_TYPE_BUILDER) {
			$this->_linkType = self::LINK_TYPE_FILE;
		}
