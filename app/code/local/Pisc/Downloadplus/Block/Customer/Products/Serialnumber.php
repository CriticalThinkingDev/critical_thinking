...
class Pisc_Downloadplus_Block_Customer_Products_Serialnumber extends Mage_Core_Block_Template
{
    ...
    protected function _prepareLayout()
    {
        ...
        if ($this->getItems()) {
            if ($this->_pager) {
                if (!$pager = $this->getLayout()->getBlock('downloadplus.customer.serialnumbers.pager')) {
    ...
    {
    	$this->setItems(Mage::getResourceModel('downloadplus/link_purchased_item_serialnumber_collection')->addFieldToFilter('serial_id',0));
...
*/
	        ...
	        $items->getSelect()
	          	->order(Array($this->getSortProducts(), $this->getSortSerialItems()));
	...
	public function getSortSerialItems() {
	    if ($this->getData('sort_serial_items')) {
	        return $this->getData('sort_serial_items');
	    }
	    return $this->_sortSerialItems;
	}
