		...
		foreach ($orderItemsCollection as $orderItem) {
			...
			foreach ($collection as $item) {
				...
				if ($item->getLinkFile() && is_file($file)) {
					$name = '<a href="' . $this->getUrl('adminhtml/downloadplus_customer_file/link', array('id' => $item->getId(), '_secure' => true)) . '">' .
	...
	public function getConfigJson($type='links')
	{
	    ...
	    if (Mage::helper('downloadplus')->isFlashUploader()) {
    		...
    		$this->getConfig()->setUrl(Mage::getModel('adminhtml/url')
    									->addSessionParam()
    									->getUrl('adminhtml/downloadplus_customer_file/upload',
    										Array(
    											'type' => $type,
	...
	public function getBrowseButtonHtml($type = '')
	{
	    ...
	    if ($block && $block->getChild('browse_button')) {
	        $html = $block->getChild('browse_button')
            	        ->setBeforeHtml('<div style="display:inline-block;" id="downloadable_link_{{id}}_' . $type . 'file-browse">')
            	        ->setAfterHtml('</div>')
