...
class Pisc_Downloadplus_Block_Adminhtml_Catalog_Product_Edit_Tab_Downloads_Additional extends Mage_Adminhtml_Block_Template
{
	...
	*/
	public function getLinkAttributes($id, $name)
	{
		...
		$block->setAttributes($helper->getCustomDownloadableAttributes($this->getProduct(), 'additional'));
	...
	{
		$helper = Mage::helper('downloadplus');
		return $helper->hasCustomDownloadableAttributes($this->getProduct(), 'additional');
	}
	...
	public function getLinkData()
	{
		...
		foreach ($collection as $item) {
			...
			if ($item->getLinkFile() && is_file($file)) {
				$name = '<a href="' . $this->getUrl('adminhtml/downloadplus_product_file/link', array('id' => $item->getId(), '_secure' => true)) . '">' .
	...
	protected function _prepareLayout()
	{
	    if (!Mage::helper('downloadplus')->isFlashUploader()) {
	        $this->setChild('uploader', $this->getLayout()->createBlock('downloadplus/uploader_single'));

	        $htmlId = $this->getChild('uploader')->getHtmlId();
	        $this->getChild('uploader')->addElementIdsMapping(Array(
	            'container' => $htmlId . '-new',
	            'delete'    => $htmlId . '-delete'
	            ));
	    }
	...
	public function getConfigJson($type='links')
	{
	    ...
	    if (Mage::helper('downloadplus')->isFlashUploader()) {
    		...
    		$this->getConfig()->setUrl(Mage::getModel('adminhtml/url')
    									->addSessionParam()
    									->getUrl('adminhtml/downloadplus_product_file/upload',
    										Array(
    											'type' => $type,
    		...
    		$config = Zend_Json::encode($this->getConfig()->getData());

	    } else {

	        $this->getUploader()->getUploaderConfig()
                    	        ->setFileParameterName($type)
                    	        ->setTarget(Mage::getModel('adminhtml/url')
                                	            ->addSessionParam()
                                	            ->getUrl('adminhtml/downloadplus_product_file/upload',
                                	                Array(
                                	                    'type' => $type,
            ...
            $this->getUploader()->getMiscConfig()->setReplaceBrowseWithRemove(true);

            $config = Mage::helper('core')->jsonEncode($this->getUploader()->getJsonConfig());
	...
	{
	    $html = null;
	    $block = $this->getUploader();
        if ($block && $block->getChild('browse_button')) {
            $html = $block->getChild('browse_button')
                       ->setBeforeHtml('<div style="display:inline-block;" id="downloadable_productlink_{{id}}_' . $type . 'file-browse">')
	                   ->setAfterHtml('</div>')
	                   ->setId('downloadable_productlink_{{id}}_' . $type . 'file-browse_button')
	                   ->toHtml();
        }
	    return $html;
	}
	...
	public function getDeleteButtonHtml($type = '')
	{
	    $html = null;
	    $block = $this->getUploader();
	    if ($block && $block->getChild('delete_button')) {
	        $html = $block->getChild('delete_button')
            	        ->setLabel('')
            	        ->setId('downloadable_productlink_{{id}}_' . $type . 'file-delete')
