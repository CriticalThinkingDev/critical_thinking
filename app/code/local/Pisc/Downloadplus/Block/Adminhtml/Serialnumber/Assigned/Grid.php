...
class Pisc_Downloadplus_Block_Adminhtml_Serialnumber_Assigned_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _prepareCollection()
    {
        ...
        $collection->addOrderToResult(Array('order_increment_id'=>'increment_id'));
    ...
    public function _customerNameFilter($collection, $column){
        ...
        if(!is_null($filterValue)){
            ...
            $filter = Array();
            $filter[] = $collection->getConnection()->quoteInto("customerFirstnameTb.value LIKE ?", '%'.$filterValue.'%');
            $filter[] = $collection->getConnection()->quoteInto("customerLastnameTb.value LIKE ?", '%'.$filterValue.'%');
            $filter[] = $collection->getConnection()->quoteInto("CONCAT(customerFirstnameTb.value, ' ', customerLastnameTb.value) LIKE ?", '%'.$filterValue.'%');
            $where = implode(' OR ', $filter);
            $collection->getSelect()->where($where);
        }
    }

    public function _orderIncrementIdFilter($collection, $column){
        $filterValue = $column->getFilter()->getValue();
        if(!is_null($filterValue)){
            $filterValue = trim($filterValue);

            $where = $collection->getConnection()->quoteInto("order.increment_id LIKE ?", '%'.$filterValue.'%');
            $collection->getSelect()->where($where);
        }
    }
...
//            'width' => '80px',
            ...
            'filter_condition_callback' => array($this, '_orderIncrementIdFilter'),
        ));
