    		...
    		if ($product->getTypeId()==Mage_Downloadable_Model_Product_Type::TYPE_DOWNLOADABLE) {
	    		if ($pools = Mage::helper('downloadplus')->getSerialnumberPoolsByProduct($product)) {
	    			...
	    			foreach($pools as $pool) {
	    				if (!empty($pool)
	    				    && $pool!=Pisc_Downloadplus_Model_Config::SERIALNUMBER_POOL_PRODUCT
	    				    && !in_array($pool, $globalPools)
	    				    && strpos($pool, Pisc_Downloadplus_Model_Config::SERIALNUMBER_POOL_GLOBAL)!==0)
	    				{
	    					$options[] = Array(
	    					    'value'=>$pool,
	    					    'label'=>Mage::helper('downloadplus')->__('%s', str_replace(Pisc_Downloadplus_Model_Config::SERIALNUMBER_POOL_GLOBAL, '', $pool))
	    					);
	    				}
	    			}
	    		}
