...
class Pisc_Downloadplus_Model_Config extends Mage_Core_Model_Config_Data
{
    ...
    /*
    const CONFIG_XML_PATH_DOWNLOADABLE_DELIVERY_FORCE_DOWNLOAD = 'catalog/downloadable_delivery/force_download';
    */
    ...
    const CONFIG_XML_PATH_DOWNLOADABLE_SERIALNUMBERS_EXPIRE_WITH_LINK = 'catalog/downloadable_serialnumbers/expire_with_link';
    ...
    const CONFIG_XML_PATH_CATALOG_PRODUCT_IMAGE_THUMBNAIL_SIZE = 'catalog/downloadable_edit/image_thumbnail_size';
    ...
    */
	 ...
	 */
	  ...
	  */
	 public function getDownloadableDeliveryProductBehaviour($object=null) {
	     if (Mage::helper('downloadplus')->existsDownloadplusRepository() && $object instanceof Mage_Downloadable_Model_Link_Purchased_Item && $object->getProductId()) {
	         if (Mage::helper('downloadplusrepository')->isProductRepositoryStyle($object->getProductId())) {
	             return self::CONFIG_BEHAVIOUR_LATEST;
	         }
	     }
	 ...
	 /*
	  * Returns if the download shall be forced in the browser
	  */
	 /*
	 public function isDownloadableDeliveryForceDownload() {
	 	return Mage::getStoreConfig(self::CONFIG_XML_PATH_DOWNLOADABLE_DELIVERY_FORCE_DOWNLOAD, $this->_storeId) == self::CONFIG_YES;
	 }
	 */
	 ...
	 */
	 ...
	 /**
	  * Gets if Serialnumbers status is same as for Link
	  */
	 public function isDownloadableSerialnumberExpireWithLink() {
	 	return Mage::getStoreConfig(self::CONFIG_XML_PATH_DOWNLOADABLE_SERIALNUMBERS_EXPIRE_WITH_LINK, $this->_storeId) == self::CONFIG_YES;
	 }
	 ...
	 */
	 ...
	 public function getImageThumbnailSize($size = null)
	 {
	 	$result = Array('75', '75');
	 	$option = Mage::getStoreConfig(self::CONFIG_XML_PATH_CATALOG_PRODUCT_IMAGE_THUMBNAIL_SIZE, $this->_storeId);
	 	if (!empty($option)) {
	 		$option = preg_replace('/\s/', '', $option);
	 		$sizes = explode('x', $option);
	 		if (count($sizes)>1) {
