...
class Pisc_Downloadplus_Model_Downloads extends Mage_Core_Model_Abstract
{
    ...
    public function getLinkUpdates()
    {
    	...
    	$cacheKey = 'store_'.$this->_store_id;
    	if ($this->_filter_product) {
    		if ($this->_filter_product instanceof Mage_Catalog_Model_Product) {
    			$cacheKey.= '_product_'.$this->_filter_product->getId();
    		} else {
    			$cacheKey.= '_product_'.$this->_filter_product;
    		}
    	}
    	if ($this->_filter_category) {
    		$cacheKey.= '_category_'.$this->_filter_category->getId();
    	}
    	if ($this->_sort) {
    		$cacheKey.= '_sort_'.$this->_sort;
    	}
    	$cackeKey = 'downloadplus_downloads_linkupdates_'.md5($cacheKey);

    	if (!is_null($this->_updatesLinks)) {
    		return $this->_updatesLinks;
    	} else {
    		if ($value = $cache->load($cacheKey)) {
    			$this->_updatesLinks = unserialize($value);
    			return $this->_updatesLinks;
    		}
    	}
    	...
    	if ($this->_filter_product) {
    		if ($this->_filter_product instanceof Mage_Catalog_Model_Product) {
    			$links->addProductToFilter($this->_filter_product->getId());
    		} else {
    		...
    		if ($link->getLinkFile()) {
    			...
    			if ($load) {
	    			...
	    			if ($file->exists()) {
		    			...
		    			$detail->loadByFile($file->getTypeBase().$file->getFile(), $this->_store_id);
			    		$data = Array(
			    			'file' => $file->getFile(),
			    			'path' => $file->getPath(),
		...
		if ($this->_sort) {
    		$this->_updatesLinks = $this->sort($this->_updatesLinks, $this->_sort, true);
		} else {
			$this->_updatesLinks = $this->sort($this->_updatesLinks, 'timestamp', false);
		}

		$cache->save(serialize($this->_updatesLinks), $cacheKey, Array('downloadplus', 'downloadplus_downloads'));
    	return $this->_updatesLinks;
    }
    ...
    {
    	$cache = Mage::app()->getCache();
    	$cacheKey = 'downloadplus_downloads_linksampleupdates_store_'.$this->_store_id;
    	...
    	if (!is_null($this->_updatesLinkSamples)) {
    		...
    		$cackeKey = 'downloadplus_downloads_linksampleupdates_'.md5($cacheKey);

    		if ($value = $cache->load($cacheKey)) {
    			$this->_updatesLinkSamples = unserialize($value);
    			return $this->_updatesLinkSamples;
    		}
    	}
    	...
    	if ($this->_filter_product) {
    		if ($this->_filter_product instanceof Mage_Catalog_Model_Product) {
    			$links = $links->addProductToFilter($this->_filter_product->getId());
    		} else {
    	...
    	if ($this->_sort) {
    		$this->_updatesLinkSamples = $this->sort($this->_updatesLinkSamples, $this->_sort, true);
    	} else {
	    	$this->_updatesLinkSamples = $this->sort($this->_updatesLinkSamples, 'timestamp', false);
    	}

    	$cache->save(serialize($this->_updatesLinkSamples), $cacheKey, Array('downloadplus', 'downloadplus_downloads'));
    	return $this->_updatesLinkSamples;
    }
    ...
    {
    	$cache = Mage::app()->getCache();
    	$cacheKey = 'downloadplus_downloads_sampleupdates_store_'.$this->_store_id;
    	...
    	if (!is_null($this->_updatesSamples)) {
    		...
    		$cackeKey = 'downloadplus_downloads_sampleupdates_'.md5($cacheKey);

    		if ($value = $cache->load($cacheKey)) {
    			$this->_updatesSamples = unserialize($value);
    			return $this->_updatesSamples;
    		}
    	}
    	...
    	$samples = Mage::getModel('downloadplus/sample')->getCollection();
    	if ($this->_filter_product) {
    		if ($this->_filter_product instanceof Mage_Catalog_Model_Product) {
    			$samples = $samples->addProductToFilter($this->_filter_product->getId());
    		} else {
    	...
    	if ($this->_sort) {
    		$this->_updatesSamples = $this->sort($this->_updatesSamples, $this->_sort, true);
    	} else {
    		$this->_updatesSamples = $this->sort($this->_updatesSamples, 'timestamp', false);
    	}
    	$cache->save(serialize($this->_updatesSamples), $cacheKey, Array('downloadplus', 'downloadplus_downloads'));
    ...
    {
    	$cache = Mage::app()->getCache();
    	$cacheKey = 'downloadplus_downloads_toplinks_store_'.$this->_store_id;

    	if (!is_null($this->_topLinks)) {
    		return $this->_topLinks;
    	} else {
    		if ($value = $cache->load($cacheKey)) {
    			$this->_topLinks = unserialize($value);
    			return $this->_topLinks;
    		}
    	}
	    ...
	    $this->_topLinks = Array();
  	  	...
  	  	$cache->save(serialize($this->_topLinks), $cacheKey, Array('downloadplus', 'downloadplus_downloads'));
    ...
    {
    	$cache = Mage::app()->getCache();
    	$cacheKey = 'downloadplus_downloads_topsamples_store_'.$this->_store_id;

    	if (!is_null($this->_topSamples)) {
    		return $this->_topSamples;
    	} else {
    		if ($value = $cache->load($cacheKey)) {
    			$this->_topSamples = unserialize($value);
    			return $this->_topSamples;
    		}
    	}
	    ...
	    foreach ($collection as $item) {
	  	...
	  	$cache->save(serialize($this->_topSamples), $cacheKey, Array('downloadplus', 'downloadplus_downloads'));
