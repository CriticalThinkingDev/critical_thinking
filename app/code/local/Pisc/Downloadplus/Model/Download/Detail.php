...
class Pisc_Downloadplus_Model_Download_Detail extends Mage_Core_Model_Abstract
{
    ...
    public function save()
    {
    	...
    	if ($this->getData('active')) {
    		$this->setData('hidden', 0);
	    	if ($this->getLinkId() || $this->getLinkSampleId() || $this->getSampleId()) {
	    		// Set other Details as historical
	    		$collection = $this->getCollection();
	    		if ($this->getLinkId()) { $collection->addFieldToFilter('link_id', Array('eq'=>$this->getLinkId())); }
	    		if ($this->getLinkSampleId()) { $collection->addFieldToFilter('link_sample_id', Array('eq'=>$this->getLinkSampleId())); }
	    		if ($this->getSampleId()) { $collection->addFieldToFilter('sample_id', Array('eq'=>$this->getSampleId())); }
	    		if ($this->getId()) { $collection->addFieldToFilter('detail_id', Array('neq'=>$this->getId())); }
    ...
    public function loadByLinkCustomerItemId($id, $store=0)
    {
    	...
    	$this->initialize();
    	$this->setLinkCustomerItemId($id);

    	return $this;
    }

    /*
     * Loads the object by the LinkProductItemId
     */
    public function loadByLinkProductItemId($id, $store=0)
    {
    	...
    	$this->initialize();
    	$this->setLinkProductItemId($id);

    	return $this;
    }

    /*
     * Loads the object by the LinkBonusItemId
     */
    public function loadByLinkBonusItemId($id, $store=0)
    {
    	if ($result = $this->getResource()->clearFilter()->addActiveToFilter()->addStoreToFilter($store)->getIdByLinkBonusItemId($id)) {
    		return $this->load($result);
    	}
    	// Load default if necessary
    	if (!is_null($store)) {
    		if ($result = $this->getResource()->clearFilter()->addActiveToFilter()->addStoreToFilter(0)->getIdByLinkBonusItemId($id)) {
    			return $this->load($result);
    		}
    	}

    	$this->initialize();
    	$this->setLinkBonusItemId($id);
   ...
   */
   ...
   public function getLinkType() {
       Mage::helper('downloadplus/download');
       return Pisc_Downloadplus_Helper_Download::LINK_TYPE_FILE;
   }
