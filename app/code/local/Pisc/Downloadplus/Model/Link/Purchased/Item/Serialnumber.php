...
class Pisc_Downloadplus_Model_Link_Purchased_Item_Serialnumber extends Mage_Core_Model_Abstract
{
    ...
    public function getLinkPurchasedItem()
    {
    	...
    	if ($this->getPurchasedItemId()) {
    		$purchasedItem->load($this->getPurchasedItemId(), 'item_id');
    	}
    	return $purchasedItem;
    }

    /*
     * Sets the reated Purchased Link Item entry
     */
   	public function setLinkPurchasedItem($item)
   	{
   		if ($item instanceof Mage_Downloadable_Model_Link_Purchased_Item && $item->getId()) {
   			$this->setPurchasedItemId($item->getId());
   		}
   	}
    ...
    public function setStatusByOrder($order=null)
    {
    	...
    	if ($order) {
	        ...
	        // Only update Status if we have a Serialnumber otherwise stay on set Status
            if ((!$config->isDownloadableSerialnumberExpireWithLink() && $this->getStatus()!=Mage_Downloadable_Model_Link_Purchased_Item::LINK_STATUS_EXPIRED)
                || $config->isDownloadableSerialnumberExpireWithLink()) {
    ...
    {
        // Only set other status as pending if we have a Serialnumber
        if ($status!=Mage_Downloadable_Model_Link_Purchased_Item::LINK_STATUS_PENDING && $this->getSerialNumber() && $this->getSerialHash()) {
            $this->setData('status', $status);
        } else {
            $this->setData('status', Mage_Downloadable_Model_Link_Purchased_Item::LINK_STATUS_PENDING);
        }
        return $this;
    }
    ...
    /*
     * Loads the model by serialnumber
     */
     ...
     */
    public function notifyCustomer($force=false)
    {
        $success = null;
        if (is_null($this->getData('customer_notified_at')) || $force) {
			...
			if ($success) {
				    			$this->setData('customer_notified_at', Mage::getSingleton('core/date')->gmtDate());
				    			$this->save();
			    			}
    		}
        }
     ...
     */
    public function notifyPending($force=false)
    {
        $success = null;
        if (is_null($this->getData('administrator_notified_at')) || $force) {
        	$customer = $this->getCustomer();
    		$order = $this->getOrder();
    		$success = false;
    		$config = Mage::getModel('downloadplus/config');
    		...
    		$success = $mailTemplate->getSentSuccess();
    		if ($success) {
    			$this->setData('administrator_notified_at', Mage::getSingleton('core/date')->gmtDate());
    			$this->save();
    		}
