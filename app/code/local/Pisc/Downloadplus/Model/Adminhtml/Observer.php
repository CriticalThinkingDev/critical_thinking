...
class Pisc_Downloadplus_Model_Adminhtml_Observer
{
	...
	public function eventAdminhtmlBlockHtmlBefore($observer)
	{
		...
		if ($block instanceof Mage_Downloadable_Block_Adminhtml_Catalog_Product_Edit_Tab_Downloadable) {
            if (!Mage::helper('downloadplus')->isFlashUploader()) {
                $block->setTemplate('downloadplus/product/edit/downloadable.phtml');
            }
		} elseif ($id) {
			switch ($id) {
		...
		if ($block instanceof Mage_Adminhtml_Block_Sales_Order_Invoice_Create_Items) {
			if ($renderer = $block->getColumnRenderer('downloadable')) {
				$parts = explode('/', $renderer->getTemplate());
				if (isset($parts[1])) {
					$file = Mage::getBaseDir('design').DS.'default'.DS.'default'.DS.str_replace('_', DS, $parts[1]).'.phtml';
					if (file_exists($file)) {
						$template = 'downloadplus/'.$parts[1];
						$renderer->setColumnRender('downloadable', $block->getBlock());
					}
				}
			}
		}
	 ...
	 * Adds Adminhtml Blocks
	 */
	public function eventAdminhtmlBlockHtmlAfter($observer)
	{
	}
	 ...
	 * Adds Core Blocks
	 */
	public function eventCoreBlockAbstractToHtmlAfter($observer)
	{
	}
	...
	public function eventCatalogProductPrepareSave($observer)
	{
		...
		// Update file details
		if ($details = $this->getValue($data['detail'], false)) {
			foreach ($details as $id=>$entry) {
			    /*
				if ($id==$this->getValue($entry['id'], null)) {
					$detail->load($id);
				}
				*/
				/*
				if (!$detail->getId()) {
				    $resource = $detail->convertTypeToResource($entry['file']);
				...
				*/
				...
				if ($id==$this->getValue($entry['id'], null)) {
                    ...
                    if ($detail->getId()) {
        				...
        				if ($this->getStoreId($observer)==$detail->getStoreId() && isset($entry['use_default']) && !is_null($entry['use_default'])) {
        					$detail->delete();
        				} else {
        					if (!isset($entry['use_default']) || (isset($entry['use_default']) && is_null($entry['use_default']))) {
        					...
        					if ($detail->getData('store_id')==$this->getStoreId($observer)) {
        						...
        						$detail->setData('version', $this->getValue($entry['fileversion'], ''));
        						$detail->setData('detail', $this->getValue($entry['filedetail'], ''));

        						/*
        						switch ($this->getValue($entry['area'], '')) {
        							case 'active':
        						...
        						*/
        						...
        						if (!$detail->isActive()) {
        							$relation = strtoupper($this->getValue($entry['relation'], ''));
        							...
        							$detail->setHidden($this->getValue($entry['hidden'], '0'));
		...
		// Update file assocation for Links & Link Images
		...
		if ($links = $this->getValue($data['settings']['link'], false)) {
			foreach ($links as $key=>$link) {
				...
				if ($link['link_id']>0) {
					...
					$extension->setData('expiry_custom_option_id', $this->getValue($link['expiry_custom_option_id'], null));
		...
		Mage::app()->cleanCache(Array('downloadplus_block_topdownloads', 'downloadplus_block_updated'));
	}
	...
	public function eventCustomerPrepareSave($observer)
	{
		...
		if ($links = $this->getValue($downloadable['link'], false)) {
			...
			foreach ($links as $link) {
				...
				if ($this->getValue($link['is_delete'], 0)==1) {
					...
					if ($this->getValue($link['file'], false)) {
		...
		// Update expiration & status data on purchased downloads
		if ($data = $this->getValue($downloadable['purchasedlink'], false)) {
			foreach ($data as $key=>$item) {
				$id = explode('|', $key);

				// Expiration
				if (isset($item['expires_on']) && ($id[0]==Pisc_Downloadplus_Model_Download_Detail::TYPE_SOURCE_PURCHASED)) {
					$extension = Mage::getModel('downloadplus/link_purchased_item_extension')->loadByItemId($id[1]);
					if ($extension->getId()) {
						$extension->setExpiresOn($item['expires_on']);
						$extension->save();
					}
				}
				...
				if (isset($item['status'])) {
					$link = null;
					switch ($id[0]) {
						case Pisc_Downloadplus_Model_Download_Detail::TYPE_SOURCE_PURCHASED:
							$link = Mage::getModel('downloadplus/link_purchased_item')->load($id[1]);
							break;
						case Pisc_Downloadplus_Model_Download_Detail::TYPE_SOURCE_BONUS:
							if (Mage::helper('downloadplus')->existsDownloadplusBonus()) {
								$link = Mage::getModel('downloadplusbonus/link_purchased_bonus_item')->load($id[1]);
							}
							break;
					}
					if ($link->getId()==$id[1]) {
						if (($link->getStatus()==Pisc_Downloadplus_Model_Link_Purchased_Item::LINK_STATUS_AVAILABLE
							...
							&& ($item['status']==Pisc_Downloadplus_Model_Link_Purchased_Item::LINK_STATUS_EXPIRED)) {
								$link->setStatus($item['status']);
								$link->save();
							}
						if ((($link->getStatus()==Pisc_Downloadplus_Model_Link_Purchased_Item::LINK_STATUS_EXPIRED
							|| $link->getStatus()==Pisc_Downloadplus_Model_Link_Purchased_Item::LINK_STATUS_PENDING
							|| $link->getStatus()==Pisc_Downloadplus_Model_Link_Purchased_Item::LINK_STATUS_PENDING_PAYMENT)
							&& $item['status']==Pisc_Downloadplus_Model_Link_Purchased_Item::LINK_STATUS_AVAILABLE)
							|| ($item['status']==Pisc_Downloadplus_Model_Config::LINK_STATUS_REFRESH)
							|| ($item['status']==Pisc_Downloadplus_Model_Config::LINK_STATUS_UPDATE)) {
								$link->setNumberOfDownloadsUsed(0);
								$link->setStatus(Pisc_Downloadplus_Model_Link_Purchased_Item::LINK_STATUS_AVAILABLE);
								$link->save();
								Mage::getModel('downloadplus/link_purchased_item_extension')
								    ->loadByPurchasedLink($link)
								    ->refreshExpiration($link);
							}
						if ($item['status']==Pisc_Downloadplus_Model_Config::LINK_STATUS_UPDATE) {
	...
	public function eventDownloadableProductUpdateLinkHistory($observer)
	{
		...
		if (isset($link['file'])) {
			...
			if ($this->getValue($link['type'], null)==Pisc_Downloadplus_Helper_Download::LINK_TYPE_FILE && $file) {
				...
				if ($this->getValue($link['link_id'], 0)>0 && $this->getValue($file['status'],'')=='new') {
		...
		if (isset($sample['file'])) {
			...
			if ($this->getValue($sample['type'], null)==Pisc_Downloadplus_Helper_Download::LINK_TYPE_FILE && $file) {
				...
				if ($this->getValue($sample['sample_id'], 0)>0 && $this->getValue($file['status'],'')=='new') {
    				// Create new association for this file
	...
	/*
	 * Preparing the Catalog Product Edit Form
	 */
	public function eventCatalogProductEditPrepareForm($observer)
	{
		$form = $observer->getEvent()->getForm();

		// Filter out the Custom Attributes for Downloadable Products (are shown separately)
		$attributes = Mage::helper('downloadplus')->getCustomDownloadableAttributes(Mage::registry('product'));
		$attributes['downloadplus_serialnr_inactive'] = true;
		$attributes['downloadplus_mag_emailtemplate'] = true;
		$attributes['downloadplus_mag_accessexpired'] = true;
		$attributes['downloadable_product_type'] = true;

		foreach ($attributes as $key=>$attribute) {
			foreach($form->getElements() as $fieldset){
				$fieldset->removeField($key);
			}
		}
	}
