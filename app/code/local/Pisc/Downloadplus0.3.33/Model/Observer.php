...
*/                
        ...
        if ($orderItemsIds) {
            $linkCustomer = Mage::getResourceModel('downloadplus/link_customer_item_collection')
            						->addFieldToFilter('order_item_id', array('in'=>$orderItemsIds));
            foreach ($linkCustomer as $link) {
        ...
        if ($orderItemsIds) {
            $linkSerial = Mage::getResourceModel('downloadplus/link_purchased_item_serialnumber_collection')
            						->addFieldToFilter('order_item_id', array('in'=>$orderItemsIds));
            foreach ($linkSerial as $link) {
                if ($link->getStatus()!=Mage_Downloadable_Model_Link_Purchased_Item::LINK_STATUS_EXPIRED) {
                	$oldStatus = $link->getStatus();
                    $link->setStatus($status)->save();
                    // Notify Customer if Link is now available
        ...
        if ($orderItemsIds) {
            $purchasedLinks = Mage::getResourceModel('downloadplus/link_purchased_item_collection')
            					->addFieldToFilter('order_item_id', array('in'=>$orderItemsIds));
            foreach ($purchasedLinks as $purchasedLink) {
            	$expiration = Mage::getModel('downloadplus/link_purchased_item_extension');
            	if ($purchasedLink->getStatus()==Mage_Downloadable_Model_Link_Purchased_Item::LINK_STATUS_AVAILABLE) {
            		$expiration->createExpirationOnOrder($purchasedLink);
            	}
        	}
        }
