...
class Pisc_Downloadplus_DownloadController extends Mage_Downloadable_DownloadController
{
	...
	protected function _processDownload($resource, $resourceType, $resourceObject=null)
    {
        ...
        if (Mage::getModel('downloadplus/config')->isDownloadableTrackProduct() && (get_class($resourceObject)=='Mage_Downloadable_Model_Link_Purchased_Item')) {
        ...
        // Record download in log
        if (Mage::getModel('downloadplus/config')->isDownloadableTrackSample() && (get_class($resourceObject)=='Mage_Downloadable_Model_Sample')) {
        ...
        if ($download->getOverrideCoreDownload()) {
        	if ($download->getRedirectUrl()) {
        		return $this->_redirect($download->getRedirectUrl());
        	}
        	return true;
        }

        session_write_close();
        if ($resourceType==Pisc_Downloadplus_Helper_Download::LINK_TYPE_AWSS3) {
        	$this->_processDownloadAWS3($download->getResourcePath(), $download->getResourceType());
        } else {
    ...
    /*
     * Process redirect to AmazonS3 for download
     */
    protected function _processDownloadAWS3($resource, $resourceType, $resourceObject=null)
    {
    ...
    public function linkActionDownloadLatest()
    {
        ...
        if ($linkPurchasedItem->getStatus() == Mage_Downloadable_Model_Link_Purchased_Item::LINK_STATUS_AVAILABLE
            ...
            if ($linkPurchasedItem->getLinkType()==Pisc_Downloadplus_Helper_Download::LINK_TYPE_URL || $linkPurchasedItem->getLinkType()==Pisc_Downloadplus_Helper_Download::LINK_TYPE_AWSS3) {
            	...
            	if ($archiveId) {
            		...
            		if ($archive->getId()==$archiveId) {
            			$resource = Mage::helper('downloadable/file')->getFilePath(
		                    Mage_Downloadable_Model_Link::getBasePath(), $archive->getFile()
            			);
            		}
            	} else {
    ...
    public function linkSampleActionDownloadLatest()
    {
        ...
        if ($link->getId()) {
            $resource = false;
            $resourceType = '';
            if ($link->getSampleType()==Pisc_Downloadplus_Helper_Download::LINK_TYPE_URL || $link->getSampleType()==Pisc_Downloadplus_Helper_Download::LINK_TYPE_AWSS3) {
            	...
            	if ($archiveId) {
            		...
            		if ($archive->getId()==$archiveId) {
            			$resource = Mage::helper('downloadable/file')->getFilePath(
		                    Mage_Downloadable_Model_Link::getBaseSamplePath(), $archive->getFile()
            			);
            		}
            	} else {
            		// Deliver current downloadable file
    ...
    public function sampleActionDownload()
    {
        ...
        if ($sample->getId()) {
            $resource = false;
            $resourceType = '';
            if ($sample->getSampleType()==Pisc_Downloadplus_Helper_Download::LINK_TYPE_URL || $sample->getSampleType()==Pisc_Downloadplus_Helper_Download::LINK_TYPE_AWSS3) {
            	...
            	if ($archiveId) {
            		...
            		if ($archive->getId()==$archiveId) {
            			$resource = Mage::helper('downloadable/file')->getFilePath(
		                    Mage_Downloadable_Model_Sample::getBasePath(), $archive->getFile()
            			);
            		}
            	} else {
            		// Deliver current downloadable file
    ...
    public function productAction()
    {
      ...
      if (! $linkProductItem->getId() ) {
      	$this->_getSession()->addNotice(Mage::helper('downloadable')->__("Requested link doesn't exist."));
      	return $this->_redirect('downloadable/download/unavailable/product/'.$linkId);
	  }
      ...
      if (!$licenseRequired ||($licenseRequired && $licenseAccepted)) {
      		...
      		if (Mage::getModel('downloadplus/config')->isDownloadableTrackProduct()) {
		        ...
		        if ( $linkProductItem->getId() ) {
	      			$log = Mage::getModel('downloadplus/log_link');
	      			$log->trackSampleLink($linkCustomerItem);
		        }
		        */
      		}
      		// Start Product download
    ...
    public function productActionDownload()
    {
    	...
    	if ($linkProductItem->getLinkType()==Pisc_Downloadplus_Helper_Download::LINK_TYPE_URL || $linkProductItem->getLinkType()==Pisc_Downloadplus_Helper_Download::LINK_TYPE_AWSS3) {
