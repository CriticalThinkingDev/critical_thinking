            ...
            $this->getSelect()
            	->columns(array('title' => new Zend_Db_Expr("
            			CASE WHEN `purchased_link_item_table`.link_title IS NOT NULL THEN `purchased_link_item_table`.link_title
	            			...
	            			WHEN `product_link_title_table`.title IS NOT NULL THEN `product_link_title_table`.title
	            			WHEN `customer_link_table`.link_title IS NOT NULL THEN `customer_link_table`.link_title
            			END
            			")))
    			->columns(array('type' => new Zend_Db_Expr("
            			CASE WHEN `main_table`.item_id IS NOT NULL THEN 'link-purchased-item'
	            			WHEN `main_table`.sample_id IS NOT NULL THEN 'sample'
	            			WHEN `main_table`.link_id IS NOT NULL THEN 'link-sample'
	            			WHEN `main_table`.product_item_id IS NOT NULL THEN 'product-link'
	            			WHEN `main_table`.customer_item_id IS NOT NULL THEN 'customer-link'
            			END
            			")))
            	...
            	->joinLeft(array('product_link_title_table' => $this->getTable('downloadplus/link_product_item_title')), '`product_link_title_table`.link_id=`main_table`.product_item_id', array())
            	->joinLeft(array('customer_link_table' => $this->getTable('downloadplus/link_customer_item')), '`customer_link_table`.item_id=`main_table`.customer_item_id', array());
