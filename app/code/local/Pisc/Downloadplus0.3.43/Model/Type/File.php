...
class Pisc_Downloadplus_Model_Type_File extends Mage_Core_Model_Abstract
{
	...
	{
		if ($link->getLinkType()==Pisc_Downloadplus_Helper_Download::LINK_TYPE_FILE 
			...
			$this->setType($link->getLinkType());
			$this->setTypeBase(Pisc_Downloadplus_Model_Download_Detail::TYPE_DOWNLOADABLE_LINK);
			if ($link->getLinkType()==Pisc_Downloadplus_Helper_Download::LINK_TYPE_FILELOCAL) {
				$this->setTypeBase(Pisc_Downloadplus_Model_Download_Detail::TYPE_DOWNLOADABLE_FILELOCAL);
			}
			return $this->loadResource(Pisc_Downloadplus_Model_Link::getBasePath($link->getLinkType()), $link->getLinkFile());
		}
		return $this->init();
	}
	...
	{
		if ($link->getSampleType()==Pisc_Downloadplus_Helper_Download::LINK_TYPE_FILE
				|| $link->getSampleType()==Pisc_Downloadplus_Helper_Download::LINK_TYPE_FILELOCAL) {
			$this->_title = $this->getLinkTitle($link);
			$this->_product = Mage::getModel('catalog/product')->load($link->getProductId());
			$this->setType($link->getSampleType());
			$this->setTypeBase(Pisc_Downloadplus_Model_Download_Detail::TYPE_DOWNLOADABLE_LINK_SAMPLE);
			if ($link->getSampleType()==Pisc_Downloadplus_Helper_Download::LINK_TYPE_FILELOCAL) {
				$this->setTypeBase(Pisc_Downloadplus_Model_Download_Detail::TYPE_DOWNLOADABLE_FILELOCAL);
			}
			return $this->loadResource(Pisc_Downloadplus_Model_Link::getBaseSamplePath($link->getLinkType()), $link->getSampleFile());
		}
		return $this->init();
	}
	...
	{
		if ($sample->getSampleType()==Pisc_Downloadplus_Helper_Download::LINK_TYPE_FILE
			...
			$this->setType($sample->getSampleType());
			$this->setTypeBase(Pisc_Downloadplus_Model_Download_Detail::TYPE_DOWNLOADABLE_SAMPLE);
			if ($sample->getSampleType()==Pisc_Downloadplus_Helper_Download::LINK_TYPE_FILELOCAL) {
				$this->setTypeBase(Pisc_Downloadplus_Model_Download_Detail::TYPE_DOWNLOADABLE_FILELOCAL);
			}
			return $this->loadResource(Pisc_Downloadplus_Model_Sample::getBasePath($sample->getSampleType()), $sample->getSampleFile());
		}
		return $this->init();
	}
