...
// Changes Introduced with 0.3.8
if (!$installer->getConnection()->tableColumnExists($this->getTable('downloadplus_download_detail'), 'link_purchased_item_id')) {
	$installer->run("
	ALTER TABLE {$this->getTable('downloadplus_download_detail')}
		CHANGE COLUMN link_purchased_item_id link_customer_item_id INT(11)
	;
	");
}
