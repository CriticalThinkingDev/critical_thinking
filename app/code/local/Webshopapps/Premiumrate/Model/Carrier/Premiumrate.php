    ...
    implements Mage_Shipping_Model_Carrier_Interface
{
    ...
    public function setRequest(Mage_Shipping_Model_Rate_Request $request)
    {
		...
		//var_dump(!$this->getConfigFlag('include_virtual_price') && $request->getAllItems());exit;
    ...
    protected function _getQuotes()
    {
        ...
        $this->setTotalPrice($request,$this->getConfigFlag('use_discount'),$this->getConfigFlag('use_tax_incl'),$this->getConfigFlag('include_free_price'));
		...
		foreach ($rateArray as $rate)
		{
			if (!empty($rate) && $rate['price'] >= 0)
			{
				...
				$method->setMethodTitle(Mage::helper('shipping')->__(str_replace('with Insurance','',$rate['delivery_type'])));
     ...
     * @return number
     */
     private function setTotalPrice($request, $discounted, $taxed, $freeInc)
     {
		$totalCart=0;
		$discount = $discounted;
		$tax = $taxed;
        ...
        if ($items != array())
        {
         	...
         	{
         		if(!$this->getConfigFlag('include_virtual_price') && $item->getProduct()->isVirtual() || $item->getParentItem())
         		{
         			continue;
         		}

         		//Free Quantity & Price Section
                if ($item->getHasChildren() && $item->isShipSeparately()) {
                    foreach ($item->getChildren() as $child) {
                        if ($child->getFreeShipping() && !$child->getProduct()->isVirtual()) {
	                        ...
	                        if($free){
		                    	$price -= $item->getQty() * $child->getPrice();
		                    	$discount ? $price -= $child->getDiscountAmount() : 0;
		                    	continue;
	                    	}

	                        if ($tax){
	                    		$price += $child->getTaxAmount();
	                    	}

		                    if($discount) {
		                    	$price -= $child->getDiscountAmount();
		                    }
                        } else {
                        	$tax ? $price += $item->getTaxAmount() : 0;
							$discount ? $price -= $item->getDiscountAmount() : 0;
                        }
                    }
            ...
            //        	$request->setPackageQty($this->oldQty - $freeQty);
 			$qty =(($this->oldQty >= $freeQty)?$this->oldQty - $freeQty:$this->oldQty);//added to resolve blank shipping rate issue
