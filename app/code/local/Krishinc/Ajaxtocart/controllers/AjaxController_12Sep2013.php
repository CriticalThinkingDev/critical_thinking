										...
										foreach($relateditems as $_rel)
										{
											...
											{	
											    ...
											    if($_mainthisProduct->getProductType() == '125') { 
													...
													/***START:: Added exception for product with sku 05235XIN as per client request**/
														if($_mainthisProduct->getSku() == '05235XIN')
														{
															$upsellProducts = $_mainthisProduct->getUpSellProductCollection()
																					->setPositionOrder()
																					->addStoreFilter();
															 if( count($upsellProducts->getItems())) {
																...
																{
																	$_related_products[]=$relUpsel->getId();
																}
																 
																if(isset($_related_products) && !empty($_related_products))
																{ 
																	foreach($_related_products as $_rel)
																	{
																		
																		$_thisProduct = Mage::getModel('catalog/product')->load($_rel);
																		if($_thisProduct->getSku() == '05235XIN-1')
																		{
																			$upsellProducts = $_thisProduct->getUpSellProductCollection()
																						->setPositionOrder()
																						->addStoreFilter();
																			$upscount = count($upsellProducts->getItems());		 							
																			if(count($upscount)>0)
																			{					
																				foreach($upsellProducts as $associatedProduct)
																				{	
							//														if($_SERVER['REMOTE_ADDR']== '202.131.115.178') {echo 'herer'.print_r($associatedProduct->getId());exit;}						
																					$_Product = Mage::getModel('catalog/product')->load($associatedProduct->getId());
