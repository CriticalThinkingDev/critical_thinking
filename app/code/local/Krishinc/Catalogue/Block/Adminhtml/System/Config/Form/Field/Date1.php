...
//    }
	...
	*/
    protected function _prepareToRender()
    {
       ...
       $this->addColumn('datefrom', array(
            'label' => Mage::helper('catalogue')->__('Date From'),
            'style' => 'width:120px',
        ));  
        
        $this->addColumn('dateto', array(
            'label' => Mage::helper('catalogue')->__('Date To'),
            'style' => 'width:120px',
        ));
	...
	*/
 	public function _renderCellTemplate($columnName)
	{
		...
		if ($columnName=="product") {
			...
			return $this->_getSearchDateFieldRenderer() 
					->setName($inputName.'[]')
					->setTitle($columnName)
					->setExtraParams('style="width:160px"') 
					->toHtml(); 
		} else if($columnName == "dateto") {
					return $this->_getSearchDateFieldRenderer() 
							->setName($inputName.'[]')
							->setTitle($columnName)
							->setExtraParams('style="width:160px"')  
	...
	*/
	...
	/**
		 ...
		 * @return Msat_Yourmodule_Block_Select
		 */
		protected function _getSearchDateFieldRenderer()
		{
			if (!$this->_searchMonthFieldRenderer) {
				$this->_searchMonthFieldRenderer = $this->getLayout()
						->createBlock('catalogue/adminhtml_date')
						->setIsRenderToJsTemplate(true);           
			}
