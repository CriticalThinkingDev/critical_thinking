    ...
    public function indexAction()
    {
        ...
        if($orderCount){
            ...
            foreach($orders as $order){
                ...
                $shippingNames = explode(" ",$order['ship_name']);
                $sfirstName = $shippingNames[0];
                $slastName = $shippingNames[1];

                $billingNames = explode(" ",$order['billing_name']);
                ...
                $shippingMethod = 'premiumrate_Standard_Shipping';
                ...
                try {
                    ...
                    $odr->setShippingAmount(0);
                    $odr->setBaseShippingAmount(0);
        ...
        if(count($educent->getData())){
            ...
            foreach($educent as $item){
                ...
                if(count($shipment) && $shipment->getTrackNumber()){
                    ...
                    $response = curl_exec($ch);
                    //p($response);
                    $emailResponseitem = array('id'=>$item->getRealOrderId(),'response'=>$response);
                    $emailResponse[] = $emailResponseitem;
                    Mage::log($response,null,'educents-track.log');
                    if($response=='success'){
                        $item->setTrack(1)->save();
            ...
            if(count($emailResponse)){
                ...
                $mailTo = "paras.sakaria@krishtechnolabs.com";
                $bcc = "paras.sakaria@krishtechnolabs.com";
                $this->sendStasticsMail($html,$mailTo,"Educents Track Cron Run",$bcc);
            }
