 ...
 * @package Krishinc_Customlinks
 */
class Krishinc_Customlinks_Block_Product_List_Otherformat extends  Mage_Catalog_Block_Product_Abstract
{
    /**
    ...
    protected function _prepareData()
    {
        ...
        $this->_itemCollection = $product->getOtherformatProductCollection()
            ->setPositionOrder()
            ->addStoreFilter()
        ;
        if (Mage::helper('catalog')->isModuleEnabled('Mage_Checkout')) {
    	 ...
    	 //echo '<pre>' ; print_r(Mage::getSingleton('catalog/config')->getProductAttributes()); 
...
//        Mage::getSingleton('catalog/product_status')->addSaleableFilterToCollection($this->_itemCollection);
        ...
        if ($this->getItemLimit('otherformat') > 0) {
            $this->_itemCollection->setPageSize($this->getItemLimit('otherformat'));
        }
         ...
         */
        Mage::dispatchEvent('catalog_product_otherformat', array(
            'product'       => $product,
    ...
    /**
     * Set how many items we need to show in otherformat block
