<?php 
class Krishinc_Catalogrequest_Block_Adminhtml_Widget_Grid extends Mage_Adminhtml_Block_Widget_Grid 
{
/**
     * Columns array
     *
    ...
    */
    ...
    public function getCsv()
    {
        ...
        foreach ($this->_columns as $column) {
            if (!$column->getIsSystem()) {
            	if(trim($column->getExportHeader())) {
        ...
        foreach ($this->getCollection() as $item) {
            ...
            foreach ($this->_columns as $column) {
                if (!$column->getIsSystem()) {  
                	...
                	$fieldValue = $column->getRowFieldExport($item);
                	$fieldValue = trim(htmlspecialchars_decode($fieldValue));
                	if($column->getIndex() == 'email'):
                	 $fieldValue =  str_replace('FAKE@CT.COM', '',$fieldValue);
                	endif;
                	if($column->getIndex() == 'created_time'):
