...
class Krishinc_Grouped_Model_Product_Type_Grouped extends Mage_Catalog_Model_Product_Type_Abstract
{ 
    ...
    public function getAssociatedProducts($product = null)
    {
        if (!$this->getProduct($product)->hasData($this->_keyAssociatedProducts)) {
            ...
            $collection = $this->getAssociatedProductCollection($product)
                ...
                ->setOrder('product_type','asc');//Added by bijal to show sort order
    ...
    { 
        ...
        foreach ($this->getAssociatedProducts($product) as $associatedProduct) {
            ...
            if($associatedProduct->isGrouped())
            {
            	foreach ($associatedProduct as $subAssociatedProduct) {
            		$salable = $salable || $subAssociatedProduct->isSalable();		
            	}
            }
        }
        return $salable; 
    ...
    protected function _prepareProduct(Varien_Object $buyRequest, $product, $processMode)
    {
        ...
        if($buyRequest->getSeriesQty())
        {
        	$productsInfo = $buyRequest->getSeriesQty();  
        }
        ...
        if (!$isStrictProcessMode || (!empty($productsInfo) && is_array($productsInfo))) {
            ...
            if ($associatedProducts || !$isStrictProcessMode) {
                foreach ($associatedProducts as $subProduct) {
					 ...
					 if($subProduct->isGrouped()){
					
					 	 $subAssociatedProducts = $this->getAssociatedProducts($subProduct);
					 	 if($subAssociatedProducts || !$isStrictProcessMode)
					 	 {
					 	 	foreach ($subAssociatedProducts as $subSubProduct) {
					 	 		$subSubProductId = $subSubProduct->getId();
			                    if(isset($productsInfo[$subProductId][$subSubProductId])) {
			                        $qty = $productsInfo[$subProductId][$subSubProductId];
			                        if (!empty($qty) && is_numeric($qty)) {
			
			                            $_result = $subSubProduct->getTypeInstance(true)
			                                ->_prepareProduct($buyRequest, $subSubProduct, $processMode);
			                            ...
			                            if ($isStrictProcessMode) {
			                                ...
			                                $_result[0]->addCustomOption('product_type', self::TYPE_CODE, $subProduct);
			                                ...
			                                $associatedProductsInfo[] = array($subSubProductId => $qty);
			                                $product->addCustomOption('associated_product_' . $subSubProductId, $qty);
    ...
    public function processBuyRequest($product, $buyRequest)
    {
        ...
        if($buyRequest->getSeriesQty())
        { 
        	$superGroup = $buyRequest->getSeriesQty(); 
        }
