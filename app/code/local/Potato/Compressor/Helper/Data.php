...
class Potato_Compressor_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    protected function _getImagesFromDir($dirPath, $optimizedImagesHashes)
    {
        ...
        foreach($regex as $filePath => $object){
            if (strpos($filePath, Potato_Compressor_Model_Compressor_Image::MEDIA_ORIGINAL_FOLDER_NAME) !== False ||
                strpos($filePath, Potato_Compressor_Model_Compressor_Image::SKIN_ORIGINAL_FOLDER_NAME) !== False
            ) {
                continue;
            }
            if (isset($optimizedImagesHashes[$this->getImageHash($filePath)]) ||
                $this->isIgnoredImage($filePath)
            ) {
                continue;
            }
