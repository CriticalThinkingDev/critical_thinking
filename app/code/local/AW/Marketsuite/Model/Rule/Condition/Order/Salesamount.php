...
class AW_Marketsuite_Model_Rule_Condition_Order_Salesamount extends Mage_Rule_Model_Condition_Combine
{
    ...
    {
        parent::__construct();
        $this->setType('marketsuite/rule_condition_order_salesamount')
            ->setValue(null);
    }
    ...
    public function getNewChildSelectOptions()
    {
        ...
        $conditions = array_merge_recursive($conditions, array(
            array('value'=>'marketsuite/rule_condition_order_salesamount_conditions|order_status', 'label'=>Mage::helper('salesrule')->__('Order status')),
            array('value'=>'marketsuite/rule_condition_order_salesamount_conditions|order_date', 'label'=>Mage::helper('salesrule')->__('Order date')),
            array('value'=>'marketsuite/rule_condition_order_salesamount_conditions|order_store_id', 'label'=>Mage::helper('salesrule')->__('Store')),
        ));
        return $conditions;
    }
    ...
    public function asHtml()
    {
        $html = $this->getTypeElement()->getHtml().
            Mage::helper('salesrule')->__("%s sales amount&nbsp; %s %s while %s of these conditions match:",
        ...
        if ($object instanceof AW_Marketsuite_Model_Customer_Abstract)
        {
            ...
            foreach ($customersOrders as $order)
                if (parent::validate($order) == (bool)$this->getValue())
                    {$count++; $price+=$order->getGrandTotal();}

            if ($this->getAttribute()=='average'&&$count!=0) $total = $price/$count;
            else $total = $price;
