 ...
 */
class AW_AdvancedReviews_Model_Mysql4_Review extends Krishinc_Review_Model_Resource_Review
{
    /**
    ...
    {        
        if ( $proscons = Mage::registry( 'advancedreviews_proscons_new_items' ) )
        {
            if ( count( $proscons ) )
            {
                $vote = Mage::getModel('advancedreviews/proscons_vote');
        ...
        # Add recommend
        if ( Mage::helper('advancedreviews')->confEnableUserDefined() && ( $value = Mage::registry( 'advancedreviews_recommend_value' ) ) )
        {
        ...
        if ($helper->isHDUActive() && $helper->isHDUEnabled() && $this->_isNewReview($object->getReviewId())){
            try {
        ...
        if ($notify->getEnabled() && $this->_isNewReview($object->getReviewId())){
     ...
     * @param Integer|String|null $review_id Review Identificator
     * @return boolean
     */
    protected function _isNewReview($review_id = null)
    {
    ...
    public function getReviewsByProscons($proscons = null,$storeId,$product_id = null,$customerId = null){
        ...
        if($product_id)
            $reviews->getSelect()->where('entity_pk_value = ?',$product_id);
