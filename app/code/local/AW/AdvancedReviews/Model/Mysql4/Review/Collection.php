...
class AW_AdvancedReviews_Model_Mysql4_Review_Collection extends Mage_Review_Model_Mysql4_Review_Collection
{
    ...
    public function setDateOrder($dir = 'DESC')
    {
        ...
        elseif ($this->_ordering === self::ORDER_BY_RATING)
        {
            ...
            if (!$this->_notIsFirst)
            {
                ...
                $this->_select->joinInner(array('ov' => $ratingTableName), '(ov.review_id = main_table.review_id)', array('summa' => 'SUM(percent)'))->group('ov.review_id');
        ...
        elseif ($this->_ordering === self::ORDER_BY_HELPFULNESS)
        {
            ...
            {                       
                $this->getSelect()
                     ->joinLeft( array( 'helpfulness' => $this->_helpfulnessTable ),
    ...
    public function getCustomerReviewsCount($customerId)
    {
        if ( $conn = $this->getConnection() )
        {
            ...
            $select = $conn->select()
                      ...
                      ->joinInner(array('detail' => $this->_reviewDetailTable), 'main_table.review_id = detail.review_id', array())
                      ->joinInner(array('store' => $this->_reviewStoreTable), 'main_table.review_id = store.review_id', array())
                      ->joinInner(array('status' => $this->_reviewStatusTable), 'main_table.status_id = status.status_id', array())
                      ->joinInner(array('entity' => $this->_reviewEntityTable), 'main_table.entity_id = entity.entity_id', array())
    ...
    public function addProsconsFilter()
    {
        if ( $proscons = Mage::helper('advancedreviews')->getAllProscons() )
        {
            ...
            {
                $this->getSelect()
                ->join( array( 'vote'.$subId => $this->_prosconsVoteTable ) ,
                        '(vote'.$subId.'.review_id = main_table.review_id AND vote'.$subId.'.proscons_id = '.$prosconsId.')' ,
                        array() );
                $subId ++;
            }
    ...
    {
        $this->getSelect()
             ->join( array( 'recommend' => $this->_recommendTable ),
