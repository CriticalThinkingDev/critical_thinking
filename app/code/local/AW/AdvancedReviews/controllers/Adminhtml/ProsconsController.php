...
class AW_AdvancedReviews_Adminhtml_ProsconsController extends Mage_Adminhtml_Controller_Action {
    ...
    public function editAction() {
        ...
        if ($model->getId() || $id == 0) {
            ...
            Mage::getSingleton('adminhtml/session')->addError($this->_getItemName() . " " . Mage::helper('advancedreviews')->__('does not exist'));
    ...
    public function deleteAction() {
        ...
        if ($id = $this->getRequest()->getParam('id')) {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess($this->_getItemName() . " " .
    ...
    public function massDeleteAction() {
        ...
        if (!is_array($proscons)) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select ') . $this->_getItemName());
        } else {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                        Mage::helper('advancedreviews')->__('Total of %d record(s) were successfully deleted', count($proscons))
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
            }
    ...
    public function massUpdateStatusAction() {
        ...
        $proscons = $this->getRequest()->getParam('proscons');
        if (!is_array($proscons)) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('advancedreviews')->__('Please select items(s)'));
        } else {
            ...
            try {
                ...
                $session->addSuccess(
                        Mage::helper('advancedreviews')->__('Total of %d record(s) were successfully updated', count($proscons))
                );
            } catch (Mage_Core_Exception $e) {
                $session->addException($e->getMessage());
            } catch (Exception $e) {
                $session->addError(Mage::helper('adminhtml')->__('Error while updating selected items(s). Please try again later.'));
            }
        }
        $this->_redirect($this->_getRedirectLink($type));
    }
