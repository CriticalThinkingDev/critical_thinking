...
//        return Mage::getStoreConfig('advancedreviews/access_options/allow_only_logged');
    ...
    public function confShowOrdering() {
        return Mage::getStoreConfig('advancedreviews/display_options/show_ordering') && Mage::getStoreConfig('advancedreviews/ordering_options/ordering_items');
    }
    ...
    public function confRecommendItemsArray() {
        ...
        if (Mage::getStoreConfig('advancedreviews/recommend_options/recommend_field1')) {
            $res_array[] = array('value' => 1, 'label' => Mage::getStoreConfig('advancedreviews/recommend_options/recommend_field1'));
        }
        if (Mage::getStoreConfig('advancedreviews/recommend_options/recommend_field2')) {
            $res_array[] = array('value' => 2, 'label' => Mage::getStoreConfig('advancedreviews/recommend_options/recommend_field2'));
        }
        if (Mage::getStoreConfig('advancedreviews/recommend_options/recommend_field3')) {
            $res_array[] = array('value' => 3, 'label' => Mage::getStoreConfig('advancedreviews/recommend_options/recommend_field3'));
        }
        if (Mage::getStoreConfig('advancedreviews/recommend_options/recommend_field4')) {
            $res_array[] = array('value' => 4, 'label' => Mage::getStoreConfig('advancedreviews/recommend_options/recommend_field4'));
        }
        if (Mage::getStoreConfig('advancedreviews/recommend_options/recommend_field5')) {
            $res_array[] = array('value' => 5, 'label' => Mage::getStoreConfig('advancedreviews/recommend_options/recommend_field5'));
        }
        return $res_array;
    }
    ...
    public function confOrderingItemsArray() {
        if ($this->confOrderingItemsAnyoneSelected()) {
            $keys_array = $this->confOrderingItemsKeysArray();
            $res_array = array();
            foreach (Mage::getModel('advancedreviews/system_config_source_ordering_items')->toOptionArray() as $item) {
                if (in_array($item['value'], $keys_array)) {
                    $res_array[] = array('value' => $item['value'], 'label' => $this->__($item['label']));
                }
            }
            return $res_array;
        } else {
    ...
    //
    // Deprecated 26.08.2009
//    public function incHelpfulness($reviewId)
//    {
//        $value = Mage::getModel('advancedreviews/helpfulness')->load($reviewId, 'review_id')->getValue();
//        Mage::getModel('advancedreviews/helpfulness')->load($reviewId, 'review_id')->setValue($value + 1)->save();
//
//        return $this;
//    }
//
//    public function decHelpfulness($reviewId)
//    {
//        $value = Mage::getModel('advancedreviews/helpfulness')->load($reviewId, 'review_id')->getValue();
//        Mage::getModel('advancedreviews/helpfulness')->load($reviewId, 'review_id')->setValue($value - 1)->save();
//
...
//    }
    ...
    public function resetProscons() {
        $session = Mage::getSingleton('customer/session', array('name' => 'frontend'))->start();
        $proscons = $session->setProscons(array());
        return $this;
    }
    ...
    public function setProsconsPlace($place) {
        $session = Mage::getSingleton('customer/session', array('name' => 'frontend'))->start();
        $reviews = $session->setProsconsPlace($place);
        return $this;
    }
    ...
    public function getFuRewiewsUrl($productId, $categoryId = null) {
        if ($path = $this->getProductRewriteUrl($productId, $categoryId))
            if (strpos($path, '.') !== false) {
                return Mage::app()->getStore()->getBaseUrl() . substr($path, 0, strpos($path, '.')) . AW_AdvancedReviews_Model_Urlrewrite::page_suffix . strstr($path, '.');
            } else {
    ...
    public function getProductRewriteUrl($productId, $categoryId = null) {
     ...
     if ($categoryId){
        ...
        else $collection->getSelect()->where('category_id IS NULL');
    ...
    public function getRssEnabled() {
        return (Mage::getStoreConfigFlag('advancedreviews/rss_options/enabled') && Mage::getStoreConfigFlag('rss/config/active')) ? true : false;
    }
    ...
    public function getFilteredReviews($product, $reviews, $page = 1, $limit = 10, $sortBy, $sortDir = 'DESC', $customer = null) {
        $block = Mage::app()->getLayout()->createBlock('advancedreviews/ajax_reviews')->setTemplate('advancedreviews/ajax/reviews.phtml');
        $collection = Mage::getResourceModel('advancedreviews/review')->getFilteredReviews($product, $reviews, $page, $limit, $sortBy, $sortDir, $customer);
        $block->setCollection($collection);
    ...
    public function getSortBar() {
        $block = Mage::app()->getLayout()->createBlock('advancedreviews/ajax_ordering')->setTemplate('advancedreviews/ajax/ordering.phtml');
        return $block->toHtml();
    }
