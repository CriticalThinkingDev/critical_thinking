...
class Mage_Sales_Model_Resource_Quote extends Mage_Sales_Model_Resource_Abstract
{
    ...
    public function substractProductFromQuotes($product)
    {
        ...
        /* added for corematchps */
        $subSelect->from(false, array(
            'items_qty'   => new Zend_Db_Expr(
                $adapter->quoteIdentifier('q.items_qty') . ' - ' . $adapter->quoteIdentifier('qi.qty')),
            'items_count' => new Zend_Db_Expr('IF('.$adapter->quoteIdentifier('q.items_count') .' > 0, '.$adapter->quoteIdentifier('q.items_count') .',1) - 1')  //new Zend_Db_Expr($adapter->quoteIdentifier('q.items_count') . ' - 1')//fix delete error
