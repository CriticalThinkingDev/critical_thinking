...
class TinyBrick_GiftCard_Model_Checkout_Cart_Observer
{
	public function saveOrderItemAttributes($observer)
	{
			...
			if($quoteItem->getIsGiftcard()) {
				if($quoteItem->getGiftcardNum() && $quoteItem->getIsGcRefill()) {
					...
					if($quoteItem->getGiftcardEmail()) {
						$customer_sendername = $order->getBillingAddress()->getFirstname();
						...
						$sent = Mage::getModel('core/email_template')
							->sendTransactional(
								...
								$quoteItem->getGiftcardEmail(), 
								array('number' => $card->getNumber(), 'amount'=> Mage::helper('core')->formatPrice($card->getBal()), 'msg' => $quoteItem->getGiftcardMsg(), 'sender_name' => $customer_sendername)
						);
						if($sent) {
							$card->setShipped(1);
						} else {
		...
		/***** Krishiinc Team - Merge code of observer to save data on onepage checkout success action***********/
		$_sourcecode_data = Mage::getSingleton('core/session')->getKrishincSourcecode(); 
		$order->setSourceCode($_sourcecode_data);
		$order->save(); 
		/***********/
	}
}