        ...
        if ($this->getRequest()->getPost()) {
            ...
            try {
                ...
                //->addData($req->getParams())
                        ...
                        ->setRegionId($req->getParam('region_id')) //Added by bijal to set region id
                        ->setRegion($req->getParam('region'))//Added by bijal to set region
                        ...
                        ->setStatus($req->getParam('status')) //added by bijal to on/off store
                        ->setCity($req->getParam('city'))
                        ->setStores($stores)
                        ->setIcon($icon);
        ...
        if ($countrycode != '') {
            ...
            if(sizeof($statearray)>0):
	            ...
	            foreach ($statearray as $_state) {
	                $state .= "<option value='" . $_state->getCode() . "'". (($region_id == $_state->getCode())?'selected="selected"':'') .">" . $_state->getDefaultName() . "</option>";
	            }
	            $state .="</select>"; 
	        else:
