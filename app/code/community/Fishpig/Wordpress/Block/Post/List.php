...
class Fishpig_Wordpress_Block_Post_List extends Mage_Core_Block_Template
{
	...
	protected function _getPagerLimit()
	{
		if (is_null($this->_pagerLimit)) {
			$this->_pagerLimit = $this->getRequest()->getParam('limit', Mage::helper('wordpress')->getWpOption('posts_per_page', 100));
		}
	...
	public function getCategoryPostCollection($categoryId)
	{
			$category = Mage::getModel('wordpress/post_category')->load($categoryId);
			if ($category) { 
				return $category->getPostCollection();
			}
			return false;
	}
}
