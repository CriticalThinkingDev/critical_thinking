...
abstract class Fishpig_Wordpress_Model_Resource_Post_Abstract extends Fishpig_Wordpress_Model_Resource_Abstract
{
	...
	public function postComment(Fishpig_Wordpress_Model_Post_Abstract $post, $name, $email, $url, $content, $extra = null)
	{
		...
		try {
			...
			$this->updatePostCommentCount($post->getId());
	...
	/**
	 * Update the number of comments a post has
	 *
	 * @param int $postId
	 * @return $this
	 */
	public function updatePostCommentCount($postId)
	{
		...
		$this->_getWriteAdapter()
			->update($this->getMainTable(), array('comment_count' => $commentCount), $this->_getWriteAdapter()->quoteInto('ID=?', $postId));
