	...
	protected function _applyCaptionShortcode(&$content, $params = array())
	{
		if (($shortcodes =	$this->_getShortcodesByTag($content, 'caption')) !== false) {
			foreach($shortcodes as $shortcode) {
				...
				$href = $this->_match('/href="(.*)"/iU', $shortcode->getInnerContent(), 1);
				$img = $this->_match('/(<img.*\/>)/iU', $shortcode->getInnerContent(), 1);
				$caption = sprintf('<p class="wp-caption-text">%s</p>', trim(strip_tags($shortcode->getInnerContent())));
				...
				$html = array(
					...
					($href ? sprintf('<a href="%s">%s</a>', $href, $img) : $img),
					$caption,
					'</div>'
				);
