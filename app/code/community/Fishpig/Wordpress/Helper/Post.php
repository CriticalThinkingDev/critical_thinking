...
class Fishpig_Wordpress_Helper_Post extends Fishpig_Wordpress_Helper_Abstract
{
		...
		if ($structure = $this->_getExplodedPermalinkStructure()) {
			...
			foreach($structure as $i => $part) {
				if (preg_match('/^\%[a-zA-Z0-9_-]{1,}\%$/', $part)) {
					...
					else if (Mage::getResourceModel('wordpress/term')->taxonomyExists($part)) {
						$part = $routerHelper->getPermalinkStringRegex();					
					}
	 ...
	 * eg. %post_id% or %postname% or %topic%etc
	 *
	 * @return false|array
	 */
	...
	public function loadByPermalink($uri)
	{
		...
		if ($loadTokens = $this->_getTokenValueArray($uri)) {
			...
			foreach($loadTokens as $token => $value) {
				...
				}//Added by bijal condition to change url as topic text
				else if ($token === 'topic') {
					$value = str_replace('-',' ',strtolower($value));
					$posts->addCustomFieldFilter('Topic', $value); 

				}
				else if (Mage::getResourceModel('wordpress/term')->taxonomyExists($token)) {
					if ($value !== $token) {
						$posts->addTermFilter($value, $token);
					}
				}
				else {
			...
			if (count($structure) > 0) {
				...
				foreach($structure as $part) {
					...
					if (preg_match('/^\%[a-zA-Z0-9_]{1,}\%$/', $part)) {
						...
						} else if($part === 'topic') { //Added by bijal condition to change url as topic text
							$url[] = str_replace(' ','-',trim(strtolower($post->getCustomField('Topic'))));
						}
						else if (Mage::getResourceModel('wordpress/term')->taxonomyExists($part)) {
							$term = Mage::getResourceModel('wordpress/term')->getTermByPostAndTaxonomy($post, $part);
