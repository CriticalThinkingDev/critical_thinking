...
class Fishpig_Wordpress_Helper_Data extends Fishpig_Wordpress_Helper_Abstract
{
	...
	public function formatDate($date, $format = null, $f = false)
	{
		 ...
		 */
		$out = '';
		if($format == 'F j, Y') //added if condition by bijal to resolve date issue(Date showing 1 day before).
		{
			$format = 'MMMM d, y';
			$out = $this->__(Mage::app()->getLocale()->date(strtotime($date), null, null, false)->toString($format) );
		} else {
	...
	public function getWordPressPath()
	{
		...
		if (is_dir($path) && is_file($path . DS . 'wp-config.php')) {
			return $path;
		}
	}
}
