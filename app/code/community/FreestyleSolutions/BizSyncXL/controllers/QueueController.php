...
************************************************************************/
class FreestyleSolutions_BizSyncXL_QueueController extends Mage_Core_Controller_Front_Action 
{
	...
	public function processAction()
	{
		...
		if ($handle = opendir( $dirname)) {
		  ...
		  $num_files = 7;
		  $stop = ($i - $num_files > 0) ? $i - $num_files : 0;
		  //  $stop = 0;
		  $total = 0;
		  
		  // 2015-03-01 PJQ - Before we start bulk update, stop indexing
		  ...
		  for($j=$i-1; $j>=$stop; $j--)
			...
			// 2015-03-01 PJQ - set indexing mode back to auto and reindex everything
			$processes = Mage::getSingleton('index/indexer')->getProcessesCollection();
