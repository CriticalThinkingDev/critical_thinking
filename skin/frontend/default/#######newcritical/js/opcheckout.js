...
Billing.prototype = {
    ...
    save: function(){
        ...
        if (validator.validate()) {
        	if (($('billing:use_for_shipping_yes')) && ($('billing:use_for_shipping_yes').checked)) {
	        	if($('billing:country_id').value.trim()=='KR'){
	        	...
	        	if(($('billing-address-select'))){
					...
					var strUser = e.options[e.selectedIndex].text.trim();
					if(strUser.indexOf('South Korea')!=-1){
						 jQuery.modal('<p><b>We do not ship to South Korea. Please contact <a href="http://www.orda.co.kr/" target="_blank">ORDA KOREA CO., LTD</a>, our exclusive licensee, to buy our products.</b></p>',{
                            dataCss: {color:'black'},
                        });
                        if(jQuery('.simplemodal-container').length!=0){
							jQuery('.simplemodal-container').addClass('shipping_country_popup');
...
//        $('billing:use_for_shipping_yes').checked = flag;
    ...
    save: function(){
        ...
        var validator = new Validation(this.form);
        if (validator.validate()) {
        	if (($('checkout-step-shipping')) && ($('shipping-new-address-form').visible())) {
	        	if($('shipping:country_id').value.trim()=='KR'){
	        		 jQuery.modal('<p><b>We do not ship to South Korea. Please contact <a href="http://www.orda.co.kr/" target="_blank">ORDA KOREA CO., LTD</a> to inquire about our products within this country.</b></p>',{
                            dataCss: {color:'black'},
                        });
                     if(jQuery('.simplemodal-container').length!=0){
...
Review.prototype = {
 ...
 //Added by bijal to restrict limit of textarea
